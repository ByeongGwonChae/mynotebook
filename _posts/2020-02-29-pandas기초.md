...
###at과 iat 사용하기
~~~ Python
df1.at['20130101', 'A'] #값으로 검색
df1.iat[0,0] #index로 검색
~~~
##loc과 iloc 사용하기
~~~ Python
df1.loc['20130101']
...
구체적인 사용법은 여기에서: https://pandas.pydata.org/pandas-docs/stable/reference/frame.html
간단한 사용 예)
~~~ Python
df.mean()
df.mean(1)
...
dtype은 기본적으로 object가 되지만, 얼마든지 변경할 수 있다.
~~~ Python
pd.Series(['a', 'b', 'c']) #default는 object
#0    a
#1    b
#2    c
#dtype: object
...
#합치기 (Merging)
##concat: 같은 형태의 자료들을 이어 하나로 합치기
~~~ Python
df = pd.DataFrame(np.random.randn(10, 4))
pieces = [df[:2], df[2:7], df[7:]]
pd.concat(pieces)
~~~
##append: concat와 비슷하지만, 보다 간략한 메소드
concat은 axis를 통해 합칠 방향을 정할 수 있지만,
append는 axis=0 방향으로 고정되어 있다.
~~~ Python
...
#그룹화
특정 컬럼을 기준으로 데이터를 그룹들로 나누고, 각 그룹에 함수를 적용할 수 있다.
~~~ Python
...
4가지 타입들이 있지만 위의 내용들만 알아도 충분할 것 같아서 따로 정리하지는 않는다.
그래도 나중에 정보가 필요할지도 모르니까 링크만 남겨 놓는다.
...
df["grade"] = df["raw_grade"].astype("category") #dtype을 object→category 로 변경
df["grade"]
#0    a
#1    b
#2    b
