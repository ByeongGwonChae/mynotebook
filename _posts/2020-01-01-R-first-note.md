...
## 5.1. 행렬의 정의
1) 행렬은 matrix( )를 사용해 표현한다.\\
2) 행렬값을 나열한뒤 ncol(열의 수) 또는 nrow(행의 수)를 사용해 행렬을 만든다.\\
...
2) ‘시작인덱스:끝인덱스’로 특정 범위의 데이터만 가져올 수도 있다.(벡터와 마찬가지)\\
3) ‘-인덱스’와 같이 표현해 특정 행/열을 제외할 수 있다.(벡터와 마찬가지)\\
4) 만약 특정 행이나 열의 전부를 가져오고 싶다면 행이나 열쪽에는 아무런 인덱스도 기재하지 않으면 된다.\\
5) 인덱스에 벡터를 넣어 나열할 수도 있다.\\
6) dimnames()를 통해 행과 열에 이름을 부여했다면 그 이름을 직접 사용할 수도 있다.
~~~ R
...
[1,]    1    7
[2,]    3    9
> (x <- matrix(c(1:9), nrow=3, dimnames=list(c('item1','item2','item3'), c('feature1','feature2','feature3'))))  #6)
      feature1 feature2 feature3
item1        1        4        7
item2        2        5        8
item3        3        6        9
...
~~~
## 6.2. 배열 데이터 접근
1) 행렬의 경우와 유사한 방식으로 각 요소에 접근할 수 있다.\\
2) 배열의 차원은 dim() 함수를 통해 알 수 있다
~~~ R
...
데이터 프레임은 행렬과 마찬가지의 모습을 하고 있지만 행렬과 달리 다양한 변수, 관측치(observations), 범주 등을 표현하기 위해 특화되어있다.
~~~ R
...
4 4  8 F
5 5 10 M
> (d$v <- c(seq(1,15,3)+2))  #이미 정의된 데이터 프레임에 새 열을 추가
[1]  3  6  9 12 15
> d
  x  y z  v
1 1  2 M  3
...
5 5 10
~~~
data.frame은 컬럼을 1개만 선택했을때 데이터 프레임이 아닌 벡터로 값을 반환한다.\\
이를 피하려면 drop=FALSE 옵션을 지정해야한다.
~~~ R
> d[,c('x'), drop=FALSE]  # 컬럼이 1개일때, 결과가 벡터로 출력되는 것을 방지
  x
1 1
2 2
3 3
...
5 5
~~~
__str()__ 함수는 R 객체의 내부 구조를 보는데 사용되는 함수로 데이터 프레임을 인자로 받으면 데이터 타입등을 잘 정리해서 한눈에 보기쉽게 나타낸다.\\
통상 R에서의 처리는 데이터 프레임을 기본 타입으로 하여 진행되므로 데이터 프레임에 많은 양의 데이터를 저장하게 된다.\\
이때 만약 단순히 데이터를 저장한 변수명을 입력하면 전체 데이터가 모두 출력되므로 데이터의 일부를 읽어보기가 어렵다. 이 경우 쉽게 데이터의 제일 앞부분만 살펴보는데 __head()__ 를 사용할 수 있다.\\
~~~ R
> (x <- data.frame(1:3))
  X1.3
11
22
33
> (colnames(x) <- c('val'))  #열 이름 지정. names()를 사용해도 같은 결과를 얻는다.
[1] "val"
...
2 2 5 8
3 3 6 9
> d[,names(d) %in% c('b','c')]  # %in% 연산자를 이용하여 특정 열만 선택
  b c
1 4 7
2 5 8
3 6 9
> d[,!names(d) %in% c('a')]  #반대로 !연산자를 사용해 특정 값들만 제외해서 열을 선택할 수도 있다.
  b c
1 4 7
2 5 8
3 6 9
~~~
# 8. 타입 판별
데이터를 처리하기 위해 여러 함수를 호출하다보면 반환된 결과의 타입이 무엇인지 분명하지 않을때가 있다.\\
이때는 class() 또는 str()를 사용해 확인해 볼 수 있다.\\
벡터와 행렬의 결과가 유사해보이지만 벡터의 경우 차원이 [1:2] (1차원에 값이 2개)라고 표시되어있는 반면, 행렬은 차원이 [1:2,1] (2차원이고 2행 1열)로 표시되어 있는 점이 다르다.\\
‘is.\*’ 형태의 함수들을 사용해 데이터의 타입을 확인 할 수 있다.[^11]
# 9. 타입 변환
1) 각 타입에 인자로 변환할 데이터를 넘긴다.
2) 또는 ‘as.\*’ 종류의 함수를 사용하여 수행할 수 있다.[^12]
~~~ R
...
[^11]: is.numeric(숫자 벡터), is.character(문자열 벡터), is.factor, is.matrix, is.data.frame 등
[^12]: as.numeric, as.factor, as.data.frame, as.matrix 등
