...
# 3. 벡터(Vector)
## 3.1. 벡터의 정의
* 벡터는 다른 프로그래밍 언어에서 흔히 보는 배열의 개념으로, c()안에 원하는 인자들을 나열하여 정의한다.
* 나열하는 인자들은 __한가지 유형__ 의 스칼라 타입이어야한다.[^4]
* 벡터는 중첩될 수 없다. 따라서 벡터 안에 벡터를 정의하면 이는 단일 차원의 벡터로 변경된다.[^5]
* 숫자형 데이터의 경우 __start:end__ 또는 __seq(from, to, by)__ 등으로 벡터를 만들 수 있다.
* seq_along(), seq_len() 등의 함수로도 벡터를 만들 수 있다.
* names() 함수를 통해 각 셀에 이름을 부여할 수 있다.
~~~ R
> (x <- c(1,2,3,4,5))  #벡터 선언
[1] 1 2 3 4 5
...
[1] 1 2 3
> seq_len(3) #seq len()은 N값이 인자로 주어지면 1:N으로 구성된 벡터를 반환한다.
[1] 1 2 3
> x <- c(1, 3, 4)  #names() 벡터의 각 셀에 원하는 이름을 부여할 수 있다.
