...
이때 만약 단순히 데이터를 저장한 변수명을 입력하면 전체 데이터가 모두 출력되므로 데이터의 일부를 읽어보기가 어렵다.\\
이 경우 __head()__ 를 사용해 데이터의 제일 앞부분만 살펴보거나, __str()__ 함수로 R 객체의 내부 구조를 요약해서 살펴본다.
~~~ R
> (x <- data.frame(1:3))
  X1.3
11
